...
description: Implements infrastructure provisioning based on approved design and tasks. Executes Terraform plans and applies changes while ensuring compliance with security and best practices.
...
2. Use multiple concurrent `speckit.implement` agent as subagent - Generate Terraform code and test in sandbox workspace (init, plan only)
...
7. Use multiple concurrent `report-tf-deployment` agent as subagent - Generate comprehensive deployment report
